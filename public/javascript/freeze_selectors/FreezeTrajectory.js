function FreezeTrajectory(selection, manualFreeze) {
    ...
    var ox = 0,
        oy = 0;
    (lx1 = 0), (ly1 = 0), (lx2 = 0), (ly2 = 0), (x1 = 0), (y1 = 0), (x2 = 0), (y2 = 0);
    ...
    if (manualFrz) {
        d3.select('body').on('keydown.freezeSelector', function() {
            ...
            if (d3.event.shiftKey) {
                ...
                manualFreezeRegion.attr('points', ox + ',' + oy + ' ' + lx1 + ',' + ly1 + ' ' + lx2 + ',' + ly2);
                ...
                manualFreezeRegion.attr('points', 0 + ',' + 0 + ' ' + 0 + ',' + 0 + ' ' + 0 + ',' + 0);
                d3.selectAll(targets).attr('id', 'untagged');
    ...
    FreezeTrajectory.redraw = function(mouse) {
        ...
        if (arguments.length > 0) {
            ...
            (x1 = prevMousePt[0]), (y1 = prevMousePt[1]), (x2 = ox), (y2 = oy);
        }
        ...
        //Update location of its clip
        if (!manualFrz) FreezeTrajectory.drawClipPath();

        //Freeze points within cursor by creating snapshots
        if (!manualFrz) FreezeTrajectory.createSnapshots([ox, oy], [lx1, ly1], [lx2, ly2]);

        //Delete snapshots outside of freeze region
        if (!manualFrz) FreezeTrajectory.cleanSnapshots([ox, oy], [lx1, ly1], [lx2, ly2], mousePt);
    };
    ...
    FreezeTrajectory.drawCursor = function() {
        if (isFinite(ox + oy + lx1 + lx2 + ly1 + ly2)) {
            freezeRegion.attr('points', ox + ',' + oy + ' ' + lx1 + ',' + ly1 + ' ' + lx2 + ',' + ly2);
        }
    };
    ...
    FreezeTrajectory.drawClipPath = function() {
        if (isFinite(ox + oy + lx1 + lx2 + ly1 + ly2)) {
            clip.attr(
                'd',
                'M ' + ox + ',' + oy + ' L ' + lx1 + ',' + ly1 + ' L ' + lx2 + ',' + ly2 + ' L ' + ox + ',' + oy
            );
        }
    };
    ...
    FreezeTrajectory.createSnapshots = function(ptA, ptB, ptC) {
        ...
        points.each(function(d, i) {
            ...
            if (det(ptA, ptB, ptD) <= 0 && det(ptA, ptC, ptD) >= 0 && pt.attr('class').includes('duped')) {
