...
var numRequiredPracticeTrials = 3; //*** how many times MUST they practice? (not optional)
var firstPracticeLength = 60 * 1000; //*** make users first practice longer
...
var practiceLengthFunc = function() {
    return Math.random() * extraPossibleTime + minTimePeriod;
};
var trialLengthFunc = function() {
    return Math.random() * extraPossibleTime + minTimePeriod;
};
...
var speeds = [{ speed: 'vary' }, { speed: 'constant' }];
...
function load(size, speed, callback) {
    ...
    for (var i = 0; i < size; i++) {
        ...
        var blueTimer = 0;
        var blueEveryMS = Math.floor(
            dotDurationMin /
                (Math.floor(Math.random() * (numBlueOnScreenRange[1] - numBlueOnScreenRange[0])) +
                    numBlueOnScreenRange[0])
        );
...
function createChart(_speed, _trail) {
    ...
    if (_trail === 'none') {
        d3
            .select('#trails')
            .html('#tagged.point {' + 'fill-opacity: 0.0 !important;' + 'stroke-opacity: 0.0 !important;' + '}');
        StreamScatterPlot.setTrails(false);
    } else if (_trail === 'trail') {
        d3
            .select('#trails')
            .html('#tagged.point {' + 'fill-opacity: 0.5 !important;' + 'stroke-opacity: 0.5 !important;' + '}');
        StreamScatterPlot.setTrails(true);
    } else {
        d3
            .select('#trails')
            .html('#tagged.snapshot {' + 'fill-opacity: 0.5 !important;' + 'stroke-opacity: 0.5 !important;' + '}');
        StreamScatterPlot.setTrails(false);
    }
...
function createQuestion(
    ...
    numpad
        ...
        .attr('class', 'numpad')
        .attr('x', function(d, i) {
            return width / 2 - numbers.length / 2 * 40 + i * 40;
        })
        .attr('y', height / 2)
        .attr('width', 35)
        .attr('height', 35)
    ...
    numpadText
        ...
        .attr('x', function(d, i) {
            return width / 2 - numbers.length / 2 * 40 + i * 40 + 35 / 2;
        })
    ...
    setTimeout(function() {
        numpad.style('fill', '#8BC34A').on('click.numpad', click_handler);
    ...
    function click_handler() {
        ...
        if (showAnswers) {
            d3
                ...
                .html(
                    '<b>Distractors that were on screen: </b>' +
                        dis +
                        '<b><br>Your answer: </b>' +
                        ans[0] +
                        '<b><br>Number of dots you clicked: </b>' +
                        dots_c
                );
        }
...
function createPractice() {
    ...
    if (_freeze === 'FreezeWholeScreen') {
        instructionsText =
            "<h3>Freeze Whole Screen</h3><p> The freeze region for this technique is the whole screen and therefore no highlighted region will be displayed.</p><img src='images/freeze_whole.png'><p><b>'Shift'</b> to freeze the all the dots on the screen.</p><p><b>'C'</b> to clear the dots you froze previously.</p>";
    } else if (_freeze === 'FreezeTrajectory') {
        instructionsText =
            "<h3>Freeze Trajectory</h3><p> The highlighted region for this technique is a cone/flashlight like region drawn in the direction of your cursors movement.</p><img src='images/freeze_trajectory.png'><p><b>'Shift'</b> to freeze the dots inside the highlighted region.</p><p><b>'C'</b> to clear the dots you froze previously.</p>";
    } else if (_freeze === 'FreezeAroundClosest') {
        instructionsText =
            "<h3>Freeze Around Closest</h3><p> The highlighted region for this technique is a circle created around the closest dot/square to your cursor.</p><img src='images/freeze_closest.png'><p><b>'Shift'</b> to freeze the dots inside the highlighted region.</p><p><b>'C'</b> to clear the dots you froze previously.</p>";
    } else if (_freeze === 'FreezeAroundCursor') {
        instructionsText =
            "<h3>Freeze Around Cursor</h3><p> The highlighted region for this technique is a circle created around your cursor.</p><img src='images/freeze_cursor.png'><p><b>'Shift'</b> to freeze the dots inside the highlighted region.</p><p><b>'C'</b> to clear the dots you froze previously.</p>";
    ...
    var text = trainInfoBox.append('div').attr('class', 'train_text');

    text.html('<b>Freeze Technique: </b>' + _freeze + '<br>' + '<b>Visual Cue: </b>' + _trail + '<br>');

    var button = trainInfoBox.append('div').attr('id', 'train_div');
...
function loadNextTrial() {
    ...
    if (practice) {
        ...
        if (practice_number > speeds.length - 1) practice_number = 0;
    } else {
        if (trialNumber == 0) {
